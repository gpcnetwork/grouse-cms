...
*/
create or replace procedure stage_diagnosis(SRC_SCHEMA STRING)
returns variant
language javascript
as
$$
...
while (tables.next())
{
    ...
    if(table.includes('MEDPAR')){
        ...
        var cols_dx = cols_raw.filter(value => {return value.includes('DGNS') && !value.includes('VRSN') && !value.includes('AD') && !value.includes('POA') && !value.includes('_E_')});
        var cols_dxtype = cols_raw.filter(value => {return value.includes('DGNS') && value.includes('VRSN') && !value.includes('AD') && !value.includes('_E_')});
        var cols_dxpoa = cols_raw.filter(value => {return value.includes('POA_DGNS') && !value.includes('_E_')});
        var cols_dx_e = cols_raw.filter(value => {return value.includes('DGNS') && !value.includes('VRSN') && !value.includes('POA') && value.includes('_E_')});
        var cols_dxtype_e = cols_raw.filter(value => {return value.includes('DGNS') && value.includes('VRSN') && value.includes('_E_')});
        var cols_dxpoa_e = cols_raw.filter(value => {return value.includes('POA_DGNS') && value.includes('_E_')});
        ...
        stg_pt_qry += `INSERT INTO `+ table +`(`+ cols +`,src_schema,src_table)
                       WITH cte_unpvt as (
                        ...
                        FROM (SELECT * FROM `+ SRC_SCHEMA +`.MEDPAR_ALL 
                              WHERE TRIM(AD_DGNS) is not null AND TRIM(AD_DGNS) <> '')
                        UNION
                        -- collect all the other diagnoses besides admitting diagnosis and E- diagnosis
                        SELECT bene_id, medparid, type_adm, sslssnf, admsndt, dschrgdt, cvrlvldt, qlfythru,
                        ...
                        FROM (SELECT * FROM `+ SRC_SCHEMA +`.MEDPAR_ALL)
                        -- multi unpivot
                        UNPIVOT (dx for dx_idx in (` + cols_dx_e + `)) dx_unpvt
                        UNPIVOT (dx_type for dx_type_idx in (` + cols_dxtype_e + `)) dxtype_unpvt    
                        UNPIVOT (dx_poa for dx_poa_idx in (` + cols_dxpoa_e + `)) dxpoa_unpvt
                        WHERE REGEXP_REPLACE(dx_idx,'[[:alpha:]]|_') = REGEXP_REPLACE(dx_type_idx,'[[:alpha:]]|_') AND
                              REGEXP_REPLACE(dx_idx,'[[:alpha:]]|_') = REGEXP_REPLACE(dx_poa_idx,'[[:alpha:]]|_') AND
                              TRIM(dx) is not null AND TRIM(dx) <> ''
                       )
    ...
    } else if(table.includes('OUTPATIENT') || table.includes('HHA') || table.includes('HOSPICE')){
