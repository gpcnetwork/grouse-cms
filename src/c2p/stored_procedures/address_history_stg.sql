...
create or replace procedure stage_private_address_history(SRC_SCHEMA STRING)
returns variant
language javascript
as
$$
/*
@param{string} SRC_SCHEMA: name of source schema
*/
...
while(get_table.next()){
    ...
    if(src_tbl.includes('MBSF')){
        ...
        var stg_qry = `INSERT INTO private_address_history_stage
                       ...
                       SELECT zip9||'|'||cnty_cd||'|'||state_cd,
                              ...
                              '` + SRC_SCHEMA + `',
                              '` + src_tbl + `',
                              to_date(replace(rfrnc_yr,',','') || '0101', 'YYYYMMDD'),
                              to_date(replace(rfrnc_yr,',','') || '1231', 'YYYYMMDD'),
                              1,
                              case when substr(zip9,6,4) = '0000' then 0.5
                                   else 1 end
                       FROM zip_clean_cte
