...
# [] empty list suggests to stage files from all requests
stage_spec = [
    #  "12518"
     "12521"
    # ,"12522"
...
# you can turn this off when all raw .dat files are staged
run_stage_step = False

# for multi-part files, you may want to run this step after confirming all file parts are seccessfully staged
run_stitch_step = True 
...
#subset staging list based on stage_spec
if len(stage_spec) > 0 and len(stage_spec) < len(cms_file_req): 
    cms_file_req = [r for r in cms_file_req for s in stage_spec if s in r]
...
with snowflake_conn as conn:
    ...
    #==== staging raw .dat files referencing .fts metadata files ====
    if run_stage_step:
    ...
    #==== stitch multi-part file together into single tables ====
    if run_stitch_step:
        #collect multi-part mapping
        ungroup_map = {}
        for fname in filenames[f"{s3_key}dat_files"]:
            fname_obj = extract.SDAFileNameParser(fname.split('.')[0])
            if(match('.*_[0-9]{3}$',fname_obj.GetTableName) and any(req in fname for req in cms_file_req)):
        ...
        for key, parts in part_map.items():
            ...
            try: 
                load.SfExec_StitchParts(conn.cursor(),key,parts,drop_after_merge=True)
            except:
                continue
